...
{  
    ...
    {
        //计算图像的直方图  
        calcHist(&srcImage, 1, &channels[c], Mat(), dstHist, dims, &size, ranges);    // cv 中是cvCalcHist  
        ...
        {  
            ...
            rectangle(dstImage,Point(i*scale, size - 1), Point((i+1)*scale - 1, size - realValue), Scalar(0,255,0),1);  
            //line(dstImage,Point(i*scale,size-1),Point((i+1)*scale-1,size-realValue),Scalar(255,255,255));  
        }  
        for(int j = 1; j < 5; j++)
        {
            line(dstImage,Point(0,(size/5) * j),Point(size*scale - 1,(size/5) * j),Scalar(0,0,255),2);  
        }
        //imshow("一维直方图", dstImage);  

        static char pic_name[] = "0.jpg";
        static char display_name[] = "one-dimensional histogram-0";
        imshow(display_name, dstImage);  
        imwrite(pic_name, dstImage);  
        pic_name[0]++;
